...
We next use `fitProlif` and `fitProlif4` to compute central 60 and 80 percent confidence limits plus 80 percent prediction, and (0.8, 0.8) tolerance limits for future nuclear proliferation, as discussed in the next three sections of this vignette.[^cipiti]
...
\centerline{\textit{\textbf{seems to threaten the extinction of civilization.}}}
\vspace{\baselineskip}
...
We want to add tolerance limits to Figures \@ref(fig:plotPred) and \@ref(fig:plotPred4) in addition to the prediction limits.  To do this, we add `rpois` simulations to the 80 percent confidence limits in those figures, rather than `rpois` simulations to _all the individual_ simulations summarized in Figures \@ref(fig:plotcum) and \@ref(fig:plotcum4).  The results for the constant-linear mixture appear in Figure \@ref(fig:plotTol).  
...
The authors wish to thank Val Nerio for suggestions that seem to have improved this article.  As usual, the authors retain responsibility for any remaining errors and poorly presented observations.  
