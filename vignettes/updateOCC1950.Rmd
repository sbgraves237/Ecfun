...
Some of the computations below are fairly long, because this dataset is so large.  This vignette is structured nearly all the R code in this vignette will be skipped in routine package testing and would only be executed if a user arranged so a file containing the desired IPUMS data extract can be found.  We do this here by creating a variable `readAndCompute` that is `FALSE` by default and is set to `TRUE` only when the the desired data are available for manual processing.  
...
`IPUMSdata` is an object with a huge number of rows and 8 colums:  
...
That's too few rows to have one row for each person in the most recent census and certainly too few to hve one row for each household in all the census since 1850:  
...
Before proceding, let's check `IPUMSdata` for missing values:  
...
In any event, after each data selection, I was given an "estimated size" for the extract (5340 MB for one trial). I entered something to "Describe your extract". Then I clicked, "Submit extract".  The IPUMS web site responded saying, "Your extract request has been submited.  You will be notfied by email at (the email address I had given them) when it has been created."  Under "Data", it said, "Processing...".  
