...
# Update <- TRUE to force the rest of this code 
# to run even if not needed to update USGDPpresidents.
...
I went to the website for [The White House President's Budget Historical Tables](https://www.whitehouse.gov/omb/budget/historical-tables/). On 2025-08-22, I saw "Historical Tables" in 3 places on that page. I clicked on the bottom one and got, `"`BUDGET-2026-HIST.xlsx`. The file I got doing this on 2025-01-22 included "Table 1.1-Summary of Receipts, Outlays, and Surpluses or Deficits (-): 1789-2026" included budget forecasts. The version of this table I got 2025-08-22 included data through 2024 but no forecasts. 
...
`R CMD build` does not like `png`. Turn it off for those purposes. If we want it, we can manually set `writePlots <- TRUE`. 
...
```{r unemployment}
selU <- !is.na(USGDPpresidents$unemployment)
plot(unemployment~Year, 
     USGDPpresidents[selU,], type='l', 
     xlab='', ylab='unemployment')
abline(h=2:3, lty='dotted', col='grey')
with(USGDPpresidents, abline(v=Year[war], 
    lty='dotted', col='light gray'))
abline(v=c(1929, 1933), col='red', lty='dotted')
text(1931, 4, 'Hoover', srt=90, col='red')
```
