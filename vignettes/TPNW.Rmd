...
This vignette was originally written to predict the date that the [Treaty on the Prohibition of Nuclear Weapons (TPNW)](https://treaties.un.org/Pages/ViewDetails.aspx?src=TREATY&mtdsg_no=XXVI-9&chapter=26&clang=_en) would enter into force.  That date has now [officially been set at 2021-01-22](https://www.un.org/sg/en/content/sg/statement/2020-10-24/un-secretary-generals-spokesman-the-occasion-of-the-50th-ratification-of-the-treaty-the-prohibition-of-nuclear-weapons).  

We now modify this vignette to include a simulation of the number of countries having officially ratifed, acceded, accepted or approved of the TPNW by that date.  

This starts by scraping the table of Participants from the web page on that treaty from the web site of the [United Nations Treaty Collection](https://treaties.un.org).  
...
On 2020-07-18 this found 24 links in 11 packages.  The first 3 packages seemed excessively specialized.^[For example, the first package found by `findFn` was [`BAwiR`](https://www.rdocumentation.org/packages/BAwiR/versions/1.2.4) for "Analysis of Basketball Data".]  The fourth was [`rvest`](https://www.rdocumentation.org/packages/rvest/versions/0.3.6) to "Easily Harvest (Scrape) Web Pages" by Hadley Wickham and others with RStudio.  
CONCERN:  This wants me to use `Selectorgadget`.  I'd prefer to try to do this without using that `Selectorgadget`.  I remember having tried to use that before and not getting it to work for me.    

A web search led me to a 2012-01-15 article on ["Scraping table from any web page with `R` or `CloudStat`"](https://www.r-bloggers.com/2012/01/scraping-table-from-any-web-page-with-r-or-cloudstat).  This suggests using [`XML::readHTMLTable`](https://www.rdocumentation.org/packages/XML/versions/3.99-0.2/topics/readHTMLTable).  
...
if(!fda::CRAN()){
  pngFile <- paste0('TPNW', today(), '.png')
#  svgFile <- 'TPNW50thRatification.svg'
#  png(pngFile, width=960, height=960)
  png(pngFile, width=960*sqrt(2), 
      height=960/sqrt(2))
  par(mar=c(6, 5, 4, 2))
