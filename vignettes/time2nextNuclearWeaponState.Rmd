...
plotNucStatesPred <- function(x=sumExpMeans, ...){
##
...
It also means that an extrapolation of the reciprocal [i.e., the Poisson mean number of first tests of new nuclear-weapon states each year] can be similar, but negatively skewed rather than positively skewed as in Figure \@ref(fig:plotfut).   
...
```{r simNew}
set.seed(9)
cumsumPred <- function(x=simMeans, ...){
##
## cumsum of rpois predictions 
##  
...
cumsumC.PI <- function(cumP=cumPred, 
            cumsumCI=cumCI, ...){
  cumPI <- sumSims(  
      nNucStates+rbind(0, cumP),
      pastfut$Year[-(1:(nYrs-1))])
...
One way to get more optimistic estimates is to pretend that the model `fitProlif1` above has 100 percent posterior probability rather than the mere `r round(fitProlif$postprob[2], 2)*100` computed with `fitProlif`.  We next look at what that estimates for future nuclear proliferation.  
...
As before, we start with simulations.  
...
And let's look at Figure \@ref(fig:cumPred1) and compare with Figure \@ref(fig:cumPred).  

```{r prolif1, fig.cap='Probability distribution of the time to more nuc-weapon states per glm model'}
probProlif1 <- plotProbs(cumPred1)
```

Figure \@ref(fig:prolif1) provides the same analysis of Figure \@ref(fig:prolif) for this simpler model with the log(Poisson mean) linear in time.  As expected, this provides a more optimistic forecast, with a `r round(max(probProlif1$probProlif)*100)` percent chance of at least one more nuclear-weapon state by `r tail(time(probProlif1$probProlif), 1)` and correspondingly lower probabilities of further nuclear proliferation more generally relative to the analysis of Figure \@ref(fig:prolif).  
