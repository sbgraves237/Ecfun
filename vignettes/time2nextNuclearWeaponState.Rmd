...
(fig:plot) Time between new nuclear nations

```{r, plot, fig.cap = "(fig:plot)"}
library(Ecdat)
...
For the first argument `y` in `ng_bsm`, we will use the number of "first tests" each year starting with 1946, the year after the "first test" by the US.  We'll use the [`lubridate` package](https://lubridate.tidyverse.org/) for dates.  The first thing we want is the current year.  We get that starting with [`today`](https://www.rdocumentation.org/packages/lubridate/versions/1.7.4/topics/today):  
