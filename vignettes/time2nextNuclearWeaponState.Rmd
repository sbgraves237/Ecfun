...
This article models the time between "first test" of a nuclear weapon by one nation and the next and uses this to obtain confidence bounds on the number of nuclear-weapon states decades into the future.  A plot of the times between "first tests" of the 9 nuclear powers as of 2019-10-15 suggests a nonhomogeneous [renewal process](https://en.wikipedia.org/wiki/Renewal_theory).  For this we use the [`bssm` package](https://www.rdocumentation.org/packages/bssm/versions/0.1.7) to model a series of annual Poisson observations of the number of states conducting a ''first test'' of a nuclear weapon each year, where the log(Poisson mean) follows a [Gaussian random walk](https://en.wikipedia.org/wiki/Random_walk#Gaussian_random_walk) in one dimension.  
...
(ref:plot) Time between new nuclear nations
...
This vignette first reviews the data and history on this issue including brief discussions of what might have motivated different nations to pursue nuclear weapons.  We then consider modeling these data as a series of annual Poisson observations of the number of states conducting a first test of a nuclear weapon each year, where the log(Poisson mean) follows a [Gaussian random walk](https://en.wikipedia.org/wiki/Random_walk#Gaussian_random_walk) in one dimension.  We model these data using the  [`ng_bsm` function](https://www.rdocumentation.org/packages/bssm/versions/0.1.8/topics/ng_bsm) in the [`bssm` package](https://www.rdocumentation.org/packages/bssm/versions/0.1.7).  This model is estimated using the [`run_mcmc` function](https://www.rdocumentation.org/packages/bssm/versions/0.1.7/topics/run_mcmc), which is further used to extend the 74 years of history of nuclear proliferation available as this is being written in 2019 into predicting another 74 years into the future with [`predict.mcmc_output`](https://www.rdocumentation.org/packages/bssm/versions/0.1.7/topics/predict.mcmc_output).  
...
Similar logic might explain why Soviet leaders might have felt a need to defend themselves from nuclear coercion after having been invaded by the US and other countries trying to put the Tsar back in power after World War I.  The United Kingdom and France likely felt nuclear threats from the Soviet Union.  

China faced nuclear threats from the US regarding Korea and the Taiwan Strait.  India faced major threats from China, [^India] Pakistan from India, [^Pak] and Israel from its neighbors. [^Israel]
...
And now the US is helping Saudi Arabia obtain nuclear power, in spite of the evidence that [the Saudi government including members of the Saudi royal family were  involved in preparations for 9-11 at least as early as 1999](https://en.wikipedia.org/wiki/The_28_pages) and their [on-going support for Al Qaeda in Yemen, reported as recently as 2019](https://en.wikipedia.org/wiki/Saudi_Arabian-led_intervention_in_Yemen). 
...
# Exponential time between 'first tests' 
...
The [`bssm` package](https://www.rdocumentation.org/packages/bssm/versions/0.1.7) provides a reasonable framework for modeling this using its [`ng_bsm` function](https://www.rdocumentation.org/packages/bssm/versions/0.1.7/topics/ng_bsm).  This function allows us to model a normal random walk in log(Poisson mean) of the number of "first tests" each year with the log(Poisson mean). This uses the fact that [the probability distribution of the time between events in a Poisson point process is exponential](https://en.wikipedia.org/wiki/Exponential_distribution).  

The first argument for `ng_bsm` is `y` = Observations as time series (or vector of class "ts") of length equal to the number of years since 1945.  We'll use the [`lubridate` package](https://lubridate.tidyverse.org/) for dates.  The first thing we want is the current year.  We get that starting with [`today`](https://www.rdocumentation.org/packages/lubridate/versions/1.7.4/topics/today):  
...
We need the other arguments of [`ng_bsm`](https://www.rdocumentation.org/packages/bssm/versions/0.1.7/topics/ng_bsm).  Many of the arguments seem fairly straightforward.  This is not true for `P1` = prior variance of `a1`.  For this we use the ["Delta method"](https://en.wikipedia.org/wiki/Delta_method) applied to `a[t] = log(mu[t])`:  For the [Poisson distribution](https://en.wikipedia.org/wiki/Poisson_distribution), the variance is equal to the mean.  And the first derivative of `log(mu)` is `1/mu`.  

Based on the above plot, let's use only the first 7 years to estimate a prior for `a1`:  These years 1946:1952 include the first tests by RU and GB:  

```{r prior}
(mu <- mean(FirstTests$nFirstTests[1:7]))
(a1 <- log(mu))
(P1 <- mu / (mu^2))
```

We also need a prior for `sd_level`, the migration standard deviation.  For this it seems sensible to start by noting that the level, alpha = log(Poisson mean), migrated from log(2/7) at the start of the period since the first test by the US to log(2/30), noting that it's been 30 years since the ["Vela Incident"](https://en.wikipedia.org/wiki/Vela_incident), which is widely believed to have been a joint test of a nuclear weapon by Israel and South Africa.  It might be plausible to assume that the difference between these numbers is one standard deviation of the sum of 74 independent normal deviatiates with a standard deviation of `sd_level`.  This assumption gives us the following estimate of `sd_level`:  

```{r sd_level}
(sd_lvl <- (abs(log(2/7)-log(2/30)) / sqrt(74)))
```

However, our confidence in this number is quite low.  The [`bssm` vignette](https://www.rdocumentation.org/packages/bssm/versions/0.1.8/vignettes/bssm.Rmd) uses the [`halfnormal`](https://www.rdocumentation.org/packages/bssm/versions/0.1.8/topics/uniform), which assumes a prior with the user specifying an `init` value and an `sd`.  For `init`, we use the value for `sd_lvl` we just computed.  For `sd` we will use a much larger number, 10 times the standard deviation of the initial prior for the level.  We will find that this very diffuse prior for `sd_level` produces estimates that look like our specification of `sd_level` may have been too small;  since we are assuming such a large number for the `sd` argument of `halfnormal` in this context, this gives us confidence that the result we are getting is coming from the data, not from specifying a prior that is too informative.  

```{r firstTstsPois}
library(bssm)
firstTstsMdl <- ng_bsm(FirstTests$nFirstTests, 
    sd_level=halfnormal(sd_lvl, 10*sqrt(P1)), 
    a1=a1, P1=P1, distribution='poisson')
str(firstTstsMdl)
```

`firstTstsMdl` is an object of class `"ng_bsm" "ngssm"`.  

# Parameter estimation 

The help page for the [`ng_bsm` function](https://www.rdocumentation.org/packages/bssm/versions/0.1.7/topics/ng_bsm) includes an example using the [`run_mcmc` function](https://www.rdocumentation.org/packages/bssm/versions/0.1.7/topics/run_mcmc) to produce simulations from which desired parameters are estimated.  Other examples included in the [`bssm` vignette](https://www.rdocumentation.org/packages/bssm/versions/0.1.8/vignettes/bssm.Rmd) provide examples estimating parameters in a model coded using both the `bsm` and `lg_ssm` functions.  

```{r run_mcmc}
set.seed(2)
firstTstsSim <- run_mcmc(firstTstsMdl, 
          nsim_states=1, n_iter = 5000)
firstTstsSim
class(firstTstsSim)
str(firstTstsSim)
```

We see that `dim(firstTstsSim$alpha)` is 75 by 1 by a number close to 645, depending on the seed of the random number generator.  The first dimension (75) is the length of the simulated time series from the year after the first test in 1945 to 2020, the year after the end of the input time series.  The second dimension is the dimensionality of the state vector, which is 1 in this case.  The third is the number of simulated changes actually retained, being a number like 645.  

In `str(firstTstsSim)`, the number of accepted simulations is `dim(firstTstsSim$alpha)[3]`, `dim(firstTstsSim$alpha)[1]`, `dim(firstTstsSim$weights)[1]`, 
`dim(firstTstsSim$counts)[1]`, and `dim(firstTstsSim$posterior)[1]`.  This is supposed to be equal to `with(firstTstsSim, 
acceptance_rate * (n_iter - n_burnin))`.  However, in a number of simulations, this latter number was sometimes 1 less than the number of actually accepted simulations;  this is a minor bug that will be corrected in future versions of `bssm`.    

With this particular simulation, we get:  

```{r acceptRate}
(nAcceptedComputed <- with(firstTstsSim, 
      acceptance_rate * (n_iter - n_burnin) ))
dim(firstTstsSim$alpha)[3]
```

Let's look at the estimated parameter, `theta`, being the unknown standard deviation of the migration rate in log(Poisson mean):  

```{r theta}
theta <- expand_sample(firstTstsSim, "theta")
library(bayesplot)
...
This is the posterior distribution of the standard deviation of the migration rate in the log(Poisson mean).  
...
The bw (bandwidth) = 0.005 was copied from an example in the [`bssm` vignette](https://www.rdocumentation.org/packages/bssm/versions/0.1.8/vignettes/bssm.Rmd).  The image here suggests that's too small.  In experimenting with this parameter, it seemed that bw = 0.1 still showed artifacts that were not replicated between different time periods, while bw = 0.3 did not show some features that were replicated between 70, 71, ..., 75 with bw = 0.2.  Therefore, I decided to use the latter:  
...
First, we plot the normal approximate confidence intervals with those estimated directly from the quantiles of the MCMC simulations.  
...
This shows something of how the `bssm` MCMC algorithm works:  A visual inspection of one simulation suggested that the first 10 of the 2500 simulated series include only 4 unique simulations.  `str(firstTstsSim)` above showed that "Length of the final jump chain = 645" for one simulation.  
...
The median goes right through the available data.  The mean looks a little high, but overall this looks like a great fit, apart from the fact that all the increments decrease log(Poisson mean).  For that we should consider adding a slope term to the model, which we consider next.  
...
sapply(firstTstsPred, quantile)

ts.plot(firstTstsPred$mean, 
        firstTstsPred$intervals, 
        col=2:5, lty=1:4)
ts.plot(firstTstsPred$mean, col=2, 
     log='y', gpars=list(las=1), xlab='')
```

```{r predict.mcmc2}
set.seed(5)
firstTstsPred5 <- try(predict(firstTstsSim,
      firstTstsFutureMdl, type='mean'))
str(firstTstsPred5)
sapply(firstTstsPred5, quantile)

ts.plot(firstTstsPred5$mean, 
        firstTstsPred5$intervals, 
        col=2:5, lty=1:4, log='y', 
        xlab='')

set.seed(6)
firstTstsPred6 <- try(predict(firstTstsSim,
      firstTstsFutureMdl, type='state'))
str(firstTstsPred6)
```

`firstTstsPred6` is still a list of length 2.  However, its second component is also a list with one component for each component of the state vector;  this model only has one component:  level.  

```{r predict.mcmc2a}
quantile(firstTstsPred6$mean)
apply(firstTstsPred6$intervals$level, 2, quantile)

ts.plot(firstTstsPred6$mean, 
        firstTstsPred6$intervals$level, 
        col=2:5, lty=1:4)
```
...
[^Pak]: [India–Pakistan relations](https://en.wikipedia.org/wiki/India%E2%80%93Pakistan_relations) have been marked by frequent conflict since the two nations were born with the dissolution of the British Raj in 1947.  This history might help people understand the need that Pakistani leaders may have felt for nuclear parity with India. 
