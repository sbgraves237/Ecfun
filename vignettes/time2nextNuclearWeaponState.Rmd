...
- Time between events might be exponentially distributed but with a hazard rate that varies over time.  [Recall that the [hazard rate](https://en.wikipedia.org/wiki/Survival_analysis#Hazard_function_and_cumulative_hazard_function) for the exponential distribution is constant: $h(t) = (-d/dt \log S(t)) = \lambda$, writing the exponential survival function as $S(t) = \exp(-\lambda t)$.]  This is consistent with the apparent increase in the time to the next "first test" by a new nuclear-weapon state.  However, the time between such "first tests" might _decrease_ in the future if more states began to perceive greater threats from other nations.  
...
matlines(nuclearWeaponStates$firstTestYr[1] + 0:nYrs, 
         exp(-predProlif), 
         lty=c('dashed', 'dotted'), 
         col=c('red', 'blue'))
...
library(Ecfun)
simMeanNucStByYr <- simulate(fitProlif, nSims, seed=3, 
    newdata=pastfut['timeSinceTrinity'], type='response')
str(simMeanNucStByYr)
```
...
```{r expSum}
expMean <- apply(1/simMeanNucStByYr, 1, mean)

expCI. <- (1/apply(simMeanNucStByYr, 1, quantile, 
                  probs=c(.1, .5, .9)) )
expCI <- cbind(estDate =as.Date(paste0(pastfut$Year, '-07-01')), 
      expMean=expMean, data.frame(t(expCI.)) )
# fix names, because the quantiles are reversed
whichCI <- which(substring(names(expCI), 1, 1) == "X")
names(expCI)[whichCI] <- c('U10', 'median', 'L10') 
```
...
xlim <- range(expCI$estDate)
ylim <- range(nuclearWeaponStates$yearsSinceLastFirstTest, 
           expMean, expCI[c('U10', 'L10')], na.rm=TRUE)
...
with(nuclearWeaponStates, 
  text(firstTest, yearsSinceLastFirstTest,
       ctry, xpd=TRUE))
with(expCI, lines(estDate, expMean))
with(expCI, lines(estDate, median, lty='dashed', col='blue'))
with(expCI, lines(estDate, U10, lty='dotted', col='red'))
with(expCI, lines(estDate, L10, lty='dotted', col='red'))
...
The fairly flat shape of the median and lower 10% lines seem consistent with a model that is `{r 100*round(fitProlif$postprob[1], 2)} percent constant and `{r 100*round(fitProlif$postprob[2], 2)} percent linear on the log scale, reported with `summary(fitProlif)` above.  The substantial curvature of the solid line forecast looks hopeful, with a mean of simulated means being almost 200 years between successive "first tests" by new nuclear-weapon states.  

However, those are simulated lognormal distributions, which means that its reciprocal may similarly have a large tail that would represent an increasing not decreasing rate of nuclear proliferation.    

A more interesting and useful view of this is to plot the evolution of the number of nuclear-weapon states through the historical period followed by the forecasts per these simulations.  For this, we assume a zero probability of a nuclear power giving up their nuclear weapons, as [South Africa did prior to its first universal elections in 1994](https://en.wikipedia.org/wiki/South_Africa#End_of_apartheid);  see Figure \@ref(fig:plotcum).  

```{r plotcum, fig.cap = "Number of nuclear-weapon states, past and predicted mean"}
str(cumMeanNucStByYr <- apply(simMeanNucStByYr[-(1:nYrs), ], 2, cumsum))
# sims are now columns rather than rows

nNucStates <- nrow(nuclearWeaponStates)
str(cumMean <- (nNucStates + 
    apply(cumMeanNucStByYr, 1, mean)))

str(cumqtle <- (nNucStates +
    apply(cumMeanNucStByYr, 1, quantile, 
                  probs=c(.1, .5, .9)) ))

str(cumCI <- cbind(estDate = as.Date(paste0(
        pastfut$Year[nYrs:(2*nYrs)], '-07-01')), 
    cumMean=c(nNucStates, cumMean), 
    data.frame(t(cbind(nNucStates, cumqtle)) ) ))
# fix names, because the quantiles are reversed
wchCI <- which(substring(names(cumCI), 1, 1) == "X")
names(cumCI)[wchCI] <- c('L10', 'median', 'U10') 

ylic <- c(0, tail(cumCI$U10, 1))

plot(xlim, ylic, type='n', xlab='', ylab='', las=1, 
     bty='n')
fT_date <- c(nuclearWeaponStates$firstTest[1], 
  nuclearWeaponStates$firstTest, Today)
lines(fT_date, 
    c(0:nNucStates, nNucStates), type='s')

with(cumCI, lines(estDate, cumMean))
with(cumCI, lines(estDate, median, lty='dashed', col='blue'))
with(cumCI, lines(estDate, U10, lty='dotted', col='red'))
with(cumCI, lines(estDate, L10, lty='dotted', col='red'))
legend('topleft', c('80% confidence level for the mean', 
    'mean', 'median'), col=c('red', 'black', 'blue'), 
    lty=c('dotted', 'solid', 'dashed'), bty='n')
```  
...
```{r simNew}
simNewNucStbyYr <- matrix(rpois(nSims*nYrs,
      as.matrix(simMeanNucStByYr[-(1:nYrs), ])), 
  nSims) 
str(simCumNewNucSt <- apply(simNewNucStbyYr, 1, cumsum))
# sims are now columns rather than rows 
quantile(simCumNewNucSt)

str(cumStQtl. <- apply(simCumNewNucSt, 1, 
        quantile, probs=c(.1, .9)) )
cumStQtl.[, 1:5]
    
cumStQtl <- ts(rbind(nNucStates, 
    nNucStates + t(cumStQtl.)), currentYear)
```
...
ylip <- c(0, tail(cumStQtl[, 2], 1))

plot(xlim, ylip, type='n', xlab='', ylab='', las=1, 
     bty='n')
...
with(cumCI, lines(estDate, U10, lty='dotted', col='red'))
with(cumCI, lines(estDate, L10, lty='dotted', col='red'))
matlines(cumCI$estDate, cumStQtl, lty='dashed', col='green')
legend('topleft', c('80% prediction interval for the future', 
        '80% confidence interval for the mean', 
        'mean', 'median'),
  col=c('green', 'red', 'black', 'blue'), 
  lty=c('dashed', 'dotted', 'solid', 'dashed'), 
  bty='n')
```  
...
str(probProlif. <- apply(simCumNewNucSt, 1, probs))
probProlif.[, 1:5]
str(probProlif <- ts(t(probProlif.), 
                     currentYear+1))

#mainProlif <- paste('Probability distribution of the time', 
#            '\nto the next 1:5 new nuclear-weapon states')
...
The image in Figure \@ref(fig:prolif) seems too pessimistic to be credible if we believe that nuclear proliferation has slowed.  This is suggested by all the above figures, but was not statistically significant in the `glm` model `PoissonFit` above.  
...
##
  ...
  coefLin <- coef(object)
  covLin <- vcov(object)
  simLin <- mvtnorm::rmvnorm(nSims, coefLin, covLin)
  
str(simMeanLin <- simLin[, 1] + 
      outer(simLin[, 2], pastfut$timeSinceTrinity))
  
  #simMeanNucStByYr <- simulate(PoissonFit, nSims, seed=3, 
#    newdata=pastfut['timeSinceTrinity'], type='response')
#   
  
}
  
tst <- simMeanNucStByYr <- simulate(PoissonFit, nSims, seed=3)
...
plot(xlim, yli1, type='n', xlab='', ylab='', las=1, 
     log='y', bty='n')
mtext(paste('Note: The US is not on this plot,', 
        '\nbecause it had no predecessors.'), 1, 3)
