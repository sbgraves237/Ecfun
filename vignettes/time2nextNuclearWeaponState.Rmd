...
We start by computing `nSims` simulated Poisson mean numbers of "first tests" by new nuclear-weapon states for each of the `nYrs` years used in `fitProlif` and `fitProlif4` and another `nYrs` years beyond.  These simulations will later be used to compute confidence intervals for the model fit and prediction and tolerance intervals for the actual number of nuclear-weapon states.  
...
dim(simMeans)
simMeans4 <- simulate(fitProlif4, nSims, seed=3, 
...
The increase over time in the _mean_ time between "first tests" in Figures \@ref(fig:plotfut) and \@ref(fig:plotfu4) looks promising.  
...
We do the same for the quartic BMA model in Figure \@ref(fig:plotcum4) to produce Figure \@ref(fig:plotPred4).    

```{r plotPred4, fig.cap = "Number of nuclear-weapon states, past and predicted"}
