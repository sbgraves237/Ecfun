...
We'll use the [`lubridate` package](https://lubridate.tidyverse.org/) for dates.  The first thing we want is the current year.  We get that starting with [`today`](https://www.rdocumentation.org/packages/lubridate/versions/1.7.4/topics/today):  
...
Let's include an observation for the current year only of it's more than 6 months since January 1 and since the last "first test":  

```{r currentYear}
if((month(Today)<7) || 
   ((Today-tail(nuclearWeaponStates$firstTest, 1))<(366/2)))
...
We need the other arguments of [`ngssm`](https://www.rdocumentation.org/packages/bssm/versions/0.1.7/topics/ngssm). Many of the arguments seem fairly straightforward.  This is not true for P1 = prior variance of a1.  For this we use the ["Delta method"](https://en.wikipedia.org/wiki/Delta_method) applied to a[t] = log(mu[t]):  For the [Poisson distribution](https://en.wikipedia.org/wiki/Poisson_distribution), the variance is equal to the mean.  And the first derivative of log(mu) = 1/mu.  

However, based on the above plot, let's use only the first 10 years in estimating a prior for a1, being 1946:1955:  This includes the first tests by RU and GB and runs not quite half the time to the first test by FR:  

```{r prior}
mu <- mean(FirstTests$nFirstTests[1:10])
a1 <- log(mu)
P1 <- mu / (mu^2)
```

In this we want to estimate R = the migration standard deviation.  Therefore, we'll set the argument R = NA and R_prior = halfnormal(abs(a1), P1):  

```{r firstTstsPois}
library(bssm)
firstTstsMdl <- ngssm(FirstTests$nFirstTests, 
    Z=1, T=1, R=NA, a1=, P1=P1, 
    distribution='poisson', R_prior=halfnormal(abs(a1), P1))
str(firstTstsMdl)
```

firstTstsMdl is an object of class "ngssm".  

The help page on the `ngssm` function does not include examples.  The `bssm` vignette @bssmV1:2017 says, "For models defined with the `ngssm`, `ng_bsm`,
`ng_ar1`, and `svm` functions, `kfilter` will first construct an approximating Gaussian model for which the Kalman filter is then used."

```{r kfilter}
str(kfilter(firstTstsMdl))
```

# Parameter estimation 

However, this does not allow us to estimate the obvious parameters in this model: the observation and migration variances.  The "bssm" vignette does not provide an example estimating parameters in an `ngssm` model, but it does provide more detail with a model coded using both the `bsm` and `lg_ssm` functions.  For that let's try the [`run_mcmc`](https://www.rdocumentation.org/packages/bssm/versions/0.1.7/topics/run_mcmc) function, copying the call from `run_mcmc(poisson_model, n_iter = 1.1e5, n_burnin = 1e4, nsim_states = 10, method = "is2")` in the bssm vignette:  

```{r run_mcmc}
set.seed(1)
firstTstsSim <- run_mcmc(firstTstsMdl, nsim_states=1, 
          n_iter = 1.1e5)
firstTstsSim
class(firstTstsSim)
```

It seems to have done what I wanted, but I'm not sure how to read the output.  

If I understand correctly the "Summary for theta", the [maximum likelihood estimate](https://en.wikipedia.org/wiki/Maximum_likelihood_estimation) (or more precisely the [maximum a posteriori estimate](https://en.wikipedia.org/wiki/Maximum_a_posteriori_estimation)) for the migration standard deviation, R is 0.186.  I'm not certain what the other numbers for theta are, but they presumably tell us something about the uncertainty in this parameter estimate.  

Similarly, if I understand correctly the "Summary for alpha_75", the posterior mean for a1 is -2.657 vs. the prior mean of -2.225.  

# Predict 

Whether I have this correct or not, the next step seems to be to extend the 74 years of history into predicting another 74 years into the future with [predict.mcmc_output](https://www.rdocumentation.org/packages/bssm/versions/0.1.7/topics/predict.mcmc_output).  For this we start by producing a `future_model`, as in the example in the help page and the bssm vignette:  

```{r future_model}
firstTstsFutureMdl <- firstTstsMdl
firstTstsFutureMdl$y <- ts(rep(NA, 74), start=end(firstTstsMdl$y))
```

```{r predict.mcmc}
firstTstsPred <- try(predict(firstTstsSim, firstTstsFutureMdl, 
        nsim=1e6))
firstTstsPred <- try(bssm:::predict.mcmc_output(firstTstsSim, firstTstsFutureMdl, 
        nsim=1e4))
# a call to nongaussian_predict omitted the last "nsim" argument. 
# Added in a local copy, calling as follows:  
#firstTstsPred <- predict.mcmc_output(firstTstsSim, 
#      firstTstsFutureMdl, nsim=1e4)
#```
#Error in nongaussian_predict(future_model, probs, t(object$theta), #object$alpha[nrow(object$alpha), : argument "nsim" is missing, with no default
# >> fixed by adding "nsim" at the end
...
#              c("ngssm", "ng_bsm", "svm", "ng_ar1")), 
#          nsim)
#
#  With this change, rerunning this call to predict.mcmc_output(
#  generated "R Session Aborted:  
#  R encounterd a fatal error.  The session was terminated.
```
